...
export default {
  ...
  methods: {
    ...
    newElement() {
      let title = this.title; // title(인풋상자내용), value 안해도 되는듯
      if (!title) {
        // title이 비었으면..
        alert("내용을 입력해주세요");
        return;
      }
      this.todoList.push({ title, completed: false }); // title에 내용이 있으면..
      this.title = ""; // 다 끝나면 값 비우기
    },
    removeTodo(idx) {
      console.log("remove"); // todoList에서 한 건 제거
